...
set -ex
  ...
  && curl --cacert "/etc/ssl/certs/ca-certificates.crt" -fsSLO --compressed "https://nodejs.org/dist/v$NODE_VERSION/node-v$NODE_VERSION-linux-$NODE_ARCH.tar.xz" \
  && curl --cacert "/etc/ssl/certs/ca-certificates.crt" -fsSLO --compressed "https://nodejs.org/dist/v$NODE_VERSION/SHASUMS256.txt.asc" \
