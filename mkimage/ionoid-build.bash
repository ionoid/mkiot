...
if [ ! -r "${mkiot_path}/ionoid-build-helpers.bash" ] ; then
        export mkiot_path="/usr/lib/mkiot/mkimage/"
fi

if [ ! -r "${mkiot_path}/ionoid-build-helpers.bash" ] ; then
        >&2 echo "Error: failed to load '${mkiot_path}/ionoid-build-helpers.bash'"
        exit 1
fi

. ${mkiot_path}/ionoid-build-helpers.bash
...
run_commands() {
        ...
        # Run commands
        "${mkiot_path}/ionoid-run-commands.py" --rootfs="$ROOTFS" \
                --buildspec="$BUILDSPEC" --phase="${phase}, $idx"
}

copy_files_to_artifact() {
        local destdir="$1"
        local idx="$2"

        info "Copying files and directories into artifact '${ARTIFACTS_BUILD_DIRECTORY}/${artifact}'"
        for i in {0..20}
        do
                local line=$(get_yaml_value "$BUILDSPEC" "$(printf %s "artifacts | .[$idx].files[$i]")")
...
generate_artifact() {
        ...
        copy_files_to_artifact "${ARTIFACTS_BUILD_DIRECTORY}/${artifacts}" "$idx"
